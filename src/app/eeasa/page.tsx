...
import { Criteria, getEeasaAccounts } from "./actions";
import { Suspense } from "react";
import Loading from "./loading";
import { AccountItem } from "@/components/AccountItem";

export const metadata: Metadata = {
  title: "Consulta los cortes del servicio eléctrico EEASA",
  description:
...
async function Accounts({ criteria, code }: SearchParams) {
  const accounts = await getEeasaAccounts({ criteria, code });

  return (
    <>
      {accounts.map((account) => (
        <AccountItem key={account.account} account={account} provider="eeasa" />
      ))}
    </>
  );
}
